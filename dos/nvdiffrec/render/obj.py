...
from . import material, mesh, texture
...
def load_obj(filename, clear_ks=True, mtl_override=None, load_materials=True):
        ...
        all_materials = [
            {
                ...
                "kd": texture.Texture2D(
                    torch.tensor([0.5, 0.5, 0.5], dtype=torch.float32, device="cuda")
                ),
    ...
    for line in lines:
        ...
        prefix = line.split()[0].lower()
        if prefix == "usemtl" and load_materials:  # Track used materials
