...
def shade(
    ...
    if gb_tangent is not None:
        ...
        cam_normal = util.safe_normalize(
            torch.matmul(gb_normal.view(b, -1, 3), w2c[:, :3, :3].transpose(2, 1))
        ).view(b, h, w, 3)
    ...
    assert (
        bsdf is not None or material.bsdf is not None
...
def render_layer(
    ...
    face_normal_indices = (
        torch.arange(0, num_faces, dtype=torch.int64, device="cuda")[:, None]
    ).repeat(1, 3)
    gb_geometric_normal, _ = interpolate(
        face_normals, rast_out_s, face_normal_indices.int()
    )

    # Compute tangent space
    # FIXME: how to get the tangents?
    ...
    if mesh.v_tng is not None:
        gb_tangent, _ = interpolate(
            mesh.v_tng, rast_out_s, mesh.t_tng_idx[0].int()
    ...
    if mesh.v_tex is not None:
        gb_texc, gb_texc_deriv = interpolate(
            mesh.v_tex, rast_out_s, mesh.t_tex_idx[0].int(), rast_db=rast_out_deriv_s
        )
    else:
    ...
    buffers = shade(
        ...
        feat=feat,
        two_sided_shading=two_sided_shading,
        delta_xy_interp=delta_xy_interp,
        dino_pred=dino_pred,
    )
...
def render_mesh(
    ...
    def composite_buffer(key, layers, background, antialias):
        ...
        for buffers, rast in reversed(layers):
            ...
            if antialias:
                accum = dr.antialias(
                    accum.contiguous(), rast, v_pos_clip, mesh.t_pos_idx[0].int()
                )
        return accum

    assert (
        mesh.t_pos_idx.shape[1] > 0
