...
from dos.networks.mlp import MLP
...
class DirectionalLight(torch.nn.Module):
    def __init__(
        ...
        mlp_layers,
        mlp_hidden_size,
        intensity_min_max=None,
        fixed_dir=None,
        fixed_intensity=None,
    ):
        super(DirectionalLight, self).__init__()
        self.mlp = MLP(mlp_in, 4, mlp_layers, nf=mlp_hidden_size, activation="sigmoid")
        if intensity_min_max is not None:
            if isinstance(intensity_min_max, list):
                intensity_min_max = torch.tensor(intensity_min_max, dtype=torch.float32)
    ...
    def forward(self, feat):
        if self.fixed_dir is not None and self.fixed_intensity is not None:
            self.light_params = torch.cat([self.fixed_dir, self.fixed_intensity], -1)
        ...
        if self.intensity_min_max is not None:
            int = (
                out[..., 2:]
                * (
                    self.intensity_min_max[1][None, :]
                    - self.intensity_min_max[0][None, :]
                )
                + self.intensity_min_max[0][None, :]
            )
