...
from . import mlptexture, texture, util
...
def load_mtl(fn, clear_ks=True):
    ...
    for line in lines:
        ...
        if "newmtl" in prefix:
            ...
            if (
                ...
                material[prefix] = torch.tensor(
                    tuple(float(d) for d in data), dtype=torch.float32, device="cuda"
                )
    ...
    for mat in materials:
        ...
        if "map_ks" in mat:
            mat["ks"] = texture.load_texture2D(
                os.path.join(mtl_path, mat["map_ks"]), channels=3
            )
        else:
        ...
        if "bump" in mat:
            mat["normal"] = texture.load_texture2D(
                os.path.join(mtl_path, mat["bump"]),
    ...
    with open(fn, "w") as f:
        ...
        if material is not None:
            ...
            if "kd_ks_normal" in material.keys():
                ...
                _, kd, ks, normal = render_uv(
                    glctx, mesh, resolution, material["kd_ks_normal"], feat=feat
                )
                batch_size = kd.shape[0]
                f.write(f"map_Kd {prefix}texture_kd.png\n")
                misc.save_images(
                    folder,
                    kd.permute(0, 3, 1, 2).detach().cpu().numpy(),
                    fnames=[prefix + "texture_kd"] * batch_size,
                )
                f.write(f"map_Ks {prefix}texture_ks.png\n")
                misc.save_images(
                    folder,
                    ks.permute(0, 3, 1, 2).detach().cpu().numpy(),
                    fnames=[prefix + "texture_ks"] * batch_size,
                )
                f.write(f"bump {prefix}texture_n.png\n")
                misc.save_images(
                    folder,
                    normal.permute(0, 3, 1, 2).detach().cpu().numpy(),
                    fnames=[prefix + "texture_n"] * batch_size,
                )
            ...
            if "ks" in material.keys():
                ...
                texture.save_texture2D(
                    os.path.join(folder, "texture_Ks.png"), material["ks"]
                )
...
def _upscale_replicate(x, full_res):
    ...
    x = torch.nn.functional.pad(
        x, (0, full_res[1] - x.shape[3], 0, full_res[0] - x.shape[2]), "replicate"
    )
...
def merge_materials(materials, texcoords, tfaces, mfaces):
    ...
    for mat in materials:
        assert (
            mat["bsdf"] == materials[0]["bsdf"]
        ), "All materials must have the same BSDF (uber shader)"
        assert ("normal" in mat) is (
            "normal" in materials[0]
        ), "All materials must have either normal map enabled or disabled"
    ...
    full_res = 2 ** np.ceil(np.log2(max_res * np.array([1, len(materials)]))).astype(
        np.int
    )
    ...
    for tex in textures:
        if tex in materials[0]:
            tex_data = torch.cat(
                tuple(
                    util.scale_img_nhwc(mat[tex].data, tuple(max_res))
                    for mat in materials
                ),
                dim=2,
        ...
        for vi in range(3):
            ...
            if not (matIdx in new_tverts[ti]):  # create new vertex
                new_tverts_data.append(
                    [
                        (matIdx + texcoords[ti][0]) / s_coeff[1],
                        texcoords[ti][1] / s_coeff[0],
                    ]
...
def initial_guess_material(cfgs, mlp=False, init_mat=None, tet_bbox=None):
    kd_min = torch.tensor(cfgs.get("kd_min", [0.0, 0.0, 0.0, 0.0]), dtype=torch.float32)
    kd_max = torch.tensor(cfgs.get("kd_max", [1.0, 1.0, 1.0, 1.0]), dtype=torch.float32)
    ks_min = torch.tensor(cfgs.get("ks_min", [0.0, 0.0, 0.0]), dtype=torch.float32)
    ks_max = torch.tensor(cfgs.get("ks_max", [0.0, 0.0, 0.0]), dtype=torch.float32)
    nrm_min = torch.tensor(cfgs.get("nrm_min", [-1.0, -1.0, 0.0]), dtype=torch.float32)
    nrm_max = torch.tensor(cfgs.get("nrm_max", [1.0, 1.0, 1.0]), dtype=torch.float32)
    if mlp:
        ...
        if cfgs.random_textures or init_mat is None:
            ...
            kd_init = (
                torch.rand(size=cfgs.texture_res + [num_channels])
                * (kd_max - kd_min)[None, None, 0:num_channels]
                + kd_min[None, None, 0:num_channels]
            )
            kd_map_opt = texture.create_trainable(
                kd_init, cfgs.texture_res, not cfgs.custom_mip, [kd_min, kd_max]
            )

            ksR = np.random.uniform(size=cfgs.texture_res + [1], low=0.0, high=0.01)
            ksG = np.random.uniform(
                size=cfgs.texture_res + [1], low=ks_min[1].cpu(), high=ks_max[1].cpu()
            )
            ksB = np.random.uniform(
                size=cfgs.texture_res + [1], low=ks_min[2].cpu(), high=ks_max[2].cpu()
            )
            ...
            kd_map_opt = texture.create_trainable(
                init_mat["kd"], cfgs.texture_res, not cfgs.custom_mip, [kd_min, kd_max]
            )
            ks_map_opt = texture.create_trainable(
                init_mat["ks"], cfgs.texture_res, not cfgs.custom_mip, [ks_min, ks_max]
            )
        ...
        mat = Material({"kd": kd_map_opt, "ks": ks_map_opt, "normal": normal_map_opt})
