...
from ..networks import MLPWithPositionalEncoding
from ..render import mesh, render
...
class DMTetGeometry(torch.nn.Module):
    def __init__(
        ...
        init_sdf=None,
        jitter_grid=0.0,
        perturb_sdf_iter=10000,
        sym_prior_shape=False,
    ):
        ...
        self.perturb_sdf_iter = perturb_sdf_iter  # TODO: shoud not be handled here
    ...
    def load_tets(self, grid_res=None, scale=None):
        ...
        )  # verts original scale (-0.5, 0.5)
        self.indices = torch.tensor(tets["indices"], dtype=torch.long, device="cuda")
        self.generate_edges()

    def get_sdf(self, pts=None, perturb_sdf=False, total_iter=0):
        ...
        if self.init_sdf is None:
            ...
            init_sdf = init_radius - pts.norm(
                dim=-1, keepdim=True
            )  # init sdf is a sphere centered at origin
            sdf = sdf + init_sdf
        elif self.init_sdf == "ellipsoid":
            rxy = self.grid_scale * 0.15
            xs, ys, zs = pts.unbind(-1)
            init_sdf = rxy - torch.stack([xs, ys, zs / 2], -1).norm(
                dim=-1, keepdim=True
            )  # init sdf is approximately an ellipsoid centered at origin
            sdf = sdf + init_sdf
        else:
            raise NotImplementedError

        if perturb_sdf:
            sdf = sdf + torch.randn_like(sdf) * 0.1 * max(
                0, 1 - total_iter / self.perturb_sdf_iter
            )
        return sdf
    ...
    def get_sdf_gradient(self):
        ...
        mesh_verts = (
            self.mesh_verts.detach()
            + (torch.rand_like(self.mesh_verts) - 0.5) * 0.1 * self.grid_scale
        )
        ...
        y = self.get_sdf(pts=sample_points, perturb_sdf=False)
    ...
    def get_sdf_reg_loss(self):
        reg_loss = {
            "sdf_bce_reg_loss": sdf_bce_reg_loss(
                self.current_sdf, self.all_edges
            ).mean()
        }
        if self.sdf_mode == "mlp":
            reg_loss["sdf_gradient_reg_loss"] = (
                (self.get_sdf_gradient().norm(dim=-1) - 1) ** 2
            ).mean()
    ...
    def getMesh(self, material=None, perturb_sdf=False, total_iter=0, jitter_grid=True):
        ...
        if jitter_grid and self.jitter_grid > 0:
            jitter = (
                (torch.rand(1, device=v_deformed.device) * 2 - 1)
                * self.jitter_grid
                * self.grid_scale
            )
            v_deformed = v_deformed + jitter

        self.current_sdf = self.get_sdf(
            v_deformed, perturb_sdf=perturb_sdf, total_iter=total_iter
        )
        verts, faces, uvs, uv_idx = self.marching_tets(
            v_deformed, self.current_sdf, self.indices
        )
        self.mesh_verts = verts
        return mesh.make_mesh(
            verts[None], faces[None], uvs[None], uv_idx[None], material
        )
